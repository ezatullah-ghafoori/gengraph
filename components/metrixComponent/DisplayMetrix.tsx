    ...
    return (
        <div className='flex  items-center justify-center my-10 pt-5'>
            ...
            <div className='flex items-center justify-center gap-2'>
                ...
                <div className='flex flex-col gap-2'>{metrix.map((row, rowIndex) => <div className='flex items-center justify-center gap-2' key={rowIndex}>
                    {row.map((col, colIndex) => <>
                        <input key={colIndex} type="text" onChange={e => handleMetrixChange && handleMetrixChange(colIndex, rowIndex, e.target.value)} value={col} className='border-1 focus:outline-none focus:bg-gray-200 rounded-md w-[40px] flex items-center justify-center text-center' />
                    </>)}
                </div>)}</div>
